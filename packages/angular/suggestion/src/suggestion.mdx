import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks'
import * as SuggestionStories from './suggestion.stories.ts'
import { OfficialDocsLink } from '../../.storybook/components/OfficialDocsLink'

<Meta of={SuggestionStories} />

# Suggestion

<OfficialDocsLink
  href="https://designsystemet.no/no/components/docs/suggestion/overview"
  component="Suggestion"
/>

`Suggestion` er en wrapper for `ds-suggestion` web-componenten fra Designsystemet for å gjøre den litt enklere å bruke i Angular-prosjekter.

<Canvas of={SuggestionStories.Preview} />
<Controls of={SuggestionStories.Preview} />

## Med forms

`Suggestion` kan brukes med Angular signal forms via `formField`.

Verdien i skjemamodellen er alltid primitiv: `string` ved enkeltvalg og `string[]` ved `multiple`. Etiketten hentes fra `ksd-suggestion-list-option`.

```ts
import { Component, signal } from '@angular/core'
import { FormField, form } from '@angular/forms/signals'
import { Field, Input, Label } from '@ks-digital/designsystem-angular/forms'
import {
  Suggestion,
  SuggestionList,
  SuggestionListEmpty,
  SuggestionListOption,
} from '@ks-digital/designsystem-angular/suggestion'

@Component({
  imports: [
    Field,
    FormField,
    Input,
    Label,
    Suggestion,
    SuggestionList,
    SuggestionListEmpty,
    SuggestionListOption,
  ],
  template: `
    <ksd-field>
      <ksd-label>Velg kommune</ksd-label>
      <ksd-suggestion [formField]="municipalityForm.municipality">
        <input ksd-input />
        <ksd-suggestion-list>
          <ksd-suggestion-list-empty>Ingen treff</ksd-suggestion-list-empty>
          @for (municipality of municipalities; track municipality.value) {
            <ksd-suggestion-list-option [value]="municipality.value">
              {{ municipality.label }}
            </ksd-suggestion-list-option>
          }
        </ksd-suggestion-list>
      </ksd-suggestion>
    </ksd-field>
  `,
})
export class App {
  municipalities = [
    { label: 'Oslo', value: '0301' },
    { label: 'Bergen', value: '4601' },
  ]

  municipalityModel = signal({ municipality: this.municipalities[0].value })
  municipalityForm = form(this.municipalityModel)
}
```

## Kontrollert bruk

- Bruk `selected` til å lagre nåværende valgt verdi (eller verdier ved `multiple`). Den godtar både `SuggestionItem` og rene verdier (`string`).
- Lytt på `selectedChange` for å reagere når brukeren velger eller fjerner et valg. Den sender alltid ut `SuggestionSelected`, altså `SuggestionItem` ved enkeltvalg og `SuggestionItem[]` ved `multiple`.
- Bruk `selected` eller `formField`, ikke begge på samme komponent.

```ts
import { Component, signal } from '@angular/core'
import { Field, Input, Label } from '@ks-digital/designsystem-angular/forms'
import {
  Suggestion,
  type SuggestionSelected,
  SuggestionList,
  SuggestionListEmpty,
  SuggestionListOption,
} from '@ks-digital/designsystem-angular/suggestion'

@Component({
  imports: [
    Field,
    Label,
    Input,
    Suggestion,
    SuggestionList,
    SuggestionListEmpty,
    SuggestionListOption,
  ],
  template: `
    <ksd-field>
      <ksd-label>Velg destinasjon</ksd-label>
      <ksd-suggestion
        [selected]="selected()"
        (selectedChange)="selected.set($event)"
      >
        <input ksd-input />
        <button type="reset" aria-label="Tøm" hidden=""></button>
        <ksd-suggestion-list>
          @for (place of places(); track place) {
            <ksd-suggestion-list-option [value]="place">{{
              place
            }}</ksd-suggestion-list-option>
          }
          <ksd-suggestion-list-empty>Ingen treff</ksd-suggestion-list-empty>
        </ksd-suggestion-list>
      </ksd-suggestion>
    </ksd-field>
  `,
})
export class App {
  places = signal(['Bergen', 'Oslo'])
  selected = signal<SuggestionSelected>(undefined)
}
```

### Eget filter

```ts
import { Component, signal } from '@angular/core'
import { Field, Input, Label } from '@ks-digital/designsystem-angular/forms'
import {
  Suggestion,
  SuggestionList,
  SuggestionListOption,
  type SuggestionFilter,
} from '@ks-digital/designsystem-angular/suggestion'

@Component({
  imports: [
    Field,
    Label,
    Input,
    Suggestion,
    SuggestionList,
    SuggestionListOption,
  ],
  template: `
    <ksd-field>
      <ksd-label>Velg kommune</ksd-label>
      <ksd-suggestion [filter]="startsWithUppercaseFilter">
        <input ksd-input />
        <button type="reset" aria-label="Tøm" hidden=""></button>
        <ksd-suggestion-list>
          @for (municipality of municipalities(); track municipality) {
            <ksd-suggestion-list-option [value]="municipality">{{
              municipality
            }}</ksd-suggestion-list-option>
          }
        </ksd-suggestion-list>
      </ksd-suggestion>
    </ksd-field>
  `,
})
export class App {
  municipalities = signal(['Bergen', 'Oslo', 'Stavanger', 'Molde'])

  startsWithUppercaseFilter: SuggestionFilter = ({ label, input }) => {
    const value = input.value.trim()
    if (!value) return true

    const firstLetter = value.charAt(0)
    const isLetter = firstLetter.toLowerCase() !== firstLetter.toUpperCase()
    const isUppercase = firstLetter === firstLetter.toUpperCase()

    if (!isLetter || !isUppercase) return false

    return label.startsWith(firstLetter)
  }
}
```

<Canvas of={SuggestionStories.CustomFiltering} />
