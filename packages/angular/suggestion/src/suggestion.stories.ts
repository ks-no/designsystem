...
export const Kommunevaapen: Story = {
  args: {},
  render: (args) => {
    const selected = signal<SuggestionItem[]>([])
    ...
    return {
      ...
      template: `
        <style>
          @import url('https://static.fiks.ks.no/img/kommunevaapen/sprites/kommunevaapen.css');

          .ksd-kommunevelger-option {
            display: inline-flex;
            align-items: center;
            gap: var(--ds-size-2);
          }

          .ksd-kommunevelger-option-icon-wrap {
            inline-size: 28px;
            block-size: 28px;
            display: inline-flex;
            align-items: center;
            ...
            <ksd-suggestion-list>
              @for (municipality of municipalities; track municipality.value) {
                <ksd-suggestion-list-option [value]="municipality.value">
                  <span class="ksd-kommunevelger-option">
                    <span class="ksd-kommunevelger-option-icon-wrap" aria-hidden="true">
                      <span
                        [class]="'ksd-kommunevelger-option-icon sprite-kommunevaapen sprite-kommunevaapen-' + municipality.value"
                      ></span>
                    </span>
                    {{ municipality.label }}
                  </span>
